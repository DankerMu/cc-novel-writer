...
- [ ] 1.1 定义风格漂移统计窗口与输入来源（最近 5 章正文/摘要）
- [ ] 1.2 实现基线对比：对齐 `style-profile.json` 的 avg_sentence_length、dialogue_ratio
- [ ] 1.3 阈值与回归规则：漂移触发（>20%/>15%）与回归清除（<10%）
...
- [ ] 3.1 定义黑名单更新协议：新增词条需 evidence（频次/例句）+ 版本/时间戳
- [ ] 3.2 误伤保护：从 style 样本提取可豁免词（或显式 whitelist）并在更新时校验
- [ ] 3.3 用户可控入口：允许手动添加/删除并在评估中提示变更

## 4. Deterministic Tooling (Optional)

- [ ] 4.1 集成 `scripts/lint-blacklist.sh`（存在则调用，不存在回退）
- [ ] 4.2 将精确 hit count/行号注入 QualityJudge context（用于可量化评分理由）
