  ...
  "issues": [
    {
      ...
      "type": "character_mapping | relationship_jump | location_contradiction | timeline_contradiction",
...
### 4.1 角色一致性（`character_mapping` / `relationship_jump`）
...
### 4.2 空间一致性（`location_contradiction`）
...
### 4.3 时间线一致性（`timeline_contradiction`，对齐 LS-001 hard 输入）
...
## 3) 提供给 QualityJudge 的结构化输入（LS-001）
