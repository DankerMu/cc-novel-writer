...
2. **全卷一致性报告（NER）**：
   - 章节范围：优先使用本卷 `outline.md` 解析得到的 `[chapter_start, chapter_end]`；若解析失败则退化为“本卷 evaluations/summaries 中出现的章节号集合”
   - 实体抽取与报告 schema：见 `skills/continue/references/continuity-checks.md`
   - 若存在 `${CLAUDE_PLUGIN_ROOT}/scripts/run-ner.sh`：逐章执行抽取；否则回退 LLM（优先 summaries，必要时回看 chapters）
